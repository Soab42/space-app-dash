    ...
    return (
      <React.Fragment>
        <tr className="border-b border-slate-200/50">
          <td className="p-4">
            <button onClick={() => toggleRow(category.id)} className="h-6 w-6 grid place-items-center rounded-md bg-slate-700/40 hover:bg-slate-900/60 border border-white/50 shadow-sm">
              <ChevronDownIcon className={`h-4 w-4 text-white transition-transform ${isRowOpen ? 'rotate-180' : ''}`} />
            </button>
          </td>
          <td className="p-4 font-medium text-slate-800 capitalize">{category.title}</td>
          ...
          <td className="p-4">
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="h-9 w-9 grid place-items-center rounded-xl bg-red-500/80 hover:bg-red-600/80 border border-white/50 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-400/50"
              ...
              <button 
                ...
                className="h-9 w-9 grid place-items-center rounded-xl bg-emerald-500/80 hover:bg-emerald-600/80 border border-white/50 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-400/50"
          ...
          <tr>
            <td colSpan={4} className="p-4 pl-12">
              <div className="rounded-xl bg-white/40 p-4">
                <h3 className="font-semibold text-slate-700 mb-2">Subcategories</h3>
                <table className="w-full">
                  <tbody>
                    {category.subcategories.map((subcategory) => (
                      <tr key={subcategory.id} className="border-b border-slate-200/50 last:border-b-0">
                        <td className="p-3 font-medium text-slate-700">{subcategory.title}</td>
                        <td className="p-3 text-slate-500">{subcategory.description}</td>
                        <td className="p-3">
                          <div className="flex items-center gap-2 justify-end">
                            ...
                            <button 
                              ...
                              className="h-8 w-8 grid place-items-center rounded-lg bg-red-500/80 hover:bg-red-600/80 border border-white/50 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-400/50"
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Categories & Subcategories</h1>
        <button 
      ...
      {/* SubCategory Modal */}
      <Modal isOpen={isSubCategoryModalOpen} onClose={() => {
        setIsSubCategoryModalOpen(false);
        setEditingSubCategory(null);
        ...
        <SubCategoryForm
          ...
          initialData={editingSubCategory ? { ...editingSubCategory, category_id: selectedCategory } : { category_id: selectedCategory }}
        />
      </Modal>

...
// dynamically assign colors by index
function colorForType(str: string) {
  let hash = 0;
...
export default function ForceGraph({
  ...
  const dataMap = graphConverter(data);
  // Build simulation nodes map
  const { nodes, nodeMap, edges } = useMemo(() => {
    const map = new Map<string, SimNode>();
    const ns: SimNode[] = dataMap.nodes.map((n, i) => ({
      ...n,
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const e of edges) {
        ...
        ctx.moveTo(tx, ty);
        ctx.lineTo(
          tx - Math.cos(ang - 0.35) * ah,
          ty - Math.sin(ang - 0.35) * ah
        );
        ctx.lineTo(
          tx - Math.cos(ang + 0.35) * ah,
          ty - Math.sin(ang + 0.35) * ah
    ...
    const drawTooltip = (
      ...
      lines.forEach((t, i) =>
        ctx.fillText(t, x + 10 + pad, y + 10 + pad + i * lh)
      );
      ctx.restore();
    };
  ...
  const fitView = useCallback(() => {
    ...
    const pad = 60;
    const parentW =
      canvas.parentElement?.getBoundingClientRect().width || canvas.width / dpr;
    const parentH = canvas.height / dpr;
    const zx = (parentW - pad) / gw;
    const zy = (parentH - pad) / gh;
    ...
    <div className="w-full border border-slate-200 rounded-2xl shadow-sm bg-slate-50/10">
      <div className="flex items-center justify-between p-2 gap-2 border-b border-slate-200 bg-slate-50 rounded-t-2xl">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => fitView()}
            className="px-3 py-1.5 rounded-lg bg-white/30 text-slate-500 border border-slate-200 hover:bg-slate-100 text-sm"
          >
            Fit
          </button>
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-white text-slate-500 border border-slate-200 hover:bg-slate-100 text-sm"
          >
            Reset
          </button>
        </div>
          ...
          <div className="absolute top-2 right-2 bg-white/90 backdrop-blur border border-slate-200 rounded-xl shadow p-3 max-w-xs text-sm">
            ...
            <div className="text-slate-600">
              ...
              <span
                style={{
                  color: colorForType(
                    nodeMap.get(selectedId!)?.type || "default"
                  ),
                }}
              >
      ...
      <div className="flex flex-wrap gap-3 p-3 text-xs text-slate-600">
        {Array.from(
          ...
          <div key={t} className="flex items-center gap-2">
            <span
              className="inline-block w-3 h-3 rounded-full text-amber-50"
...
export const graphConverter = (data: { nodes: any[]; edges: any[] }) => {
  // {"id": "HumanCellAtlas", "label": "Human Cell Atlas", "type": "Concept"},
  const nodes = data.nodes.map(
    (node: { id: string; label: string; tooltip: string }) => {
      return {
        id: node.id,
        label: node.label,
        type: node.tooltip,
  ...
  // {"source": "HumanCellAtlas", "target": "SpaceOmics", "relationship": "proposed as a"},
